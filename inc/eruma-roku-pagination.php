...
function eruma_roku_pagination( $before = '', $after = '' ) {
	...
	echo $before.'<nav class="pagination is-centered" aria-label="pagination">'."";

	$prevposts = get_previous_posts_link('Previous');
	if( $prevposts ){
		echo $prevposts;
	} else {
		echo '<a class="pagination-previous" title="'. esc_attr('This is the first page', 'eruma-roku') .'" disabled>'. esc_html('Previous', 'eruma-roku') .'</a>';
	}
	...
	if( $nextposts ){
		...
		echo '<a class="pagination-next" title="'. esc_attr('This is the last page', 'eruma-roku') .'" disabled>'. esc_html('Next page', 'eruma-roku') .'</a>';
	}

	echo '<ul class="pagination-list">';
	for( $i = $start_page; $i <= $end_page; $i++){
        if( $i == $paged ){
            echo '<li><a href="#" class="pagination-link is-current" aria-label="Page '. $i .'" aria-current="page">'.$i.'</a></li>';
        } else {
            echo '<li><a class="pagination-link" href="'.get_pagenum_link($i).'" aria-label="Go to page '. $i .'">'.$i.'</a></li>';
        }
	}
